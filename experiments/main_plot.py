...
# Change k
df = pd.read_csv("/home/pengxinbian/Desktop/experiment/Main/change_k_dna200.csv")
...
for index, row in df.iterrows():
    tau.append(row['tau'])
    k.append(row['k'])
    space.append(row['Max RSS (kB)'] / 700000)
...
plt.bar(k, space, widths, color='orange', label = 'RPM-ST')
...
plt.bar(k, quality, widths, color='green', label = 'RPM-ST')
...
## Change tau
df = pd.read_csv("/home/pengxinbian/Desktop/experiment/Main/change_tau_dna200.csv")
...
plt.bar(tau, space, widths, color='orange', label = 'RPM-ST')
...
plt.bar(tau, quality, widths, color='green', label = 'RPM-ST')
...
# plt.bar(n, runtime, color = 'skyblue', label = 'RPM-ST', width = 20)
# # plt.gca().xaxis.set_major_formatter(ScalarFormatter())
# # plt.ticklabel_format(style='plain')
# plt.ylim(0, max(runtime) + 200)
# plt.gca().yaxis.get_major_formatter().set_useOffset(False)
