...
int main(int argv, char** argc) {
    ...
    INT k = stoi(argc[3]); 
    // Input the maximum number of versions we want to check
    INT FILE_NUM = stoi(argc[4]);   // wiki
    ...
    cout << "tau = " << freq_threshold << ", k = " << k << ", M = " << M <<  endl;
    ...
    INT target_freq_ver = FILE_NUM, target_resi_ver = FILE_NUM;
    INT init_freq_size = 0, init_resi_size = 0;


    INT TAR_FLAG = 0;
    for (INT ver = 1; ver <= FILE_NUM; ver++) {
        ...
        for(const auto &item : first_freq_set) {
            if(freq_set.find(item) != freq_set.end()) {
                ...
                cout << "Losing fingerprint = " << item << endl;
            }
        }
        ...
        if (target_resi_ver != FILE_NUM && target_freq_ver != FILE_NUM) {
            cout << "*******Find target_resi_ver =  " << target_resi_ver << ", target_freq_ver = "<< target_freq_ver << "*********" << endl;
            output_stream.open("version_result.csv", ios::app);
            if(!output_stream.is_open()) {
                cout << "Couldn't open output file\n" << endl; 
            }

            output_stream << to_string(freq_threshold) << "," << to_string(k) << "," << to_string(M) << "," << target_freq_ver << "," << target_resi_ver << "\n";
            output_stream.close();
            TAR_FLAG = 1;
            break;
        }
    ...
    if (TAR_FLAG == 0) {
        ...
        output_stream << to_string(freq_threshold) << "," << to_string(k) << "," << to_string(M) << "," << to_string(target_freq_ver) << "," << to_string(target_resi_ver) << "\n";
        output_stream.close();
    }
