...
int main(int argv, char** argc) {
    ...
    while (is_text.read(reinterpret_cast<char*>(&chr), 1)) {
		if(chr == '\n') {
            ...
            ofstream tmp_file("resi_line_tmp", ios::binary);
            if (tmp_file.is_open()) {
                string str(reinterpret_cast<char*>(txt_string), text_size);
                tmp_file << str;
                tmp_file << '\n';
                tmp_file.close();
            } else {
            ...
            // // Run the current str in MAIN algorithm
            string command_str = "./main resi_line_tmp " + to_string(freq_threshold) + " " + to_string(k);
            ...
            string line;
            INT index = 0;
            unordered_set<string> cur_resi_set;
            ...
            if (file.is_open()) {
                while (getline(file, line)) {
                    ...
                    if (str.length() >= 3) {
                        resi_set.insert(str);  // Record all resilient patterns among all string lines
                        cur_resi_set.insert(str);  // Record resilient patterns for the current string
                    }
            ...
            // FINISH reading the resilient patterns from the current string
            ...
            vector<INT> occ;
            for(auto const &pattern : cur_resi_set) {
                unsigned char* target = const_cast<unsigned char*>(
    ...
    for (INT i = 1; i < string_index; i++) {
        map<string,INT> cur_row_map = frequency_matrix[i];
        for (const auto &pattern : resi_set) {
    ...
    // Write to CSV
    ofstream nor_file("normalized_resi_matrix.csv");
    if (nor_file.is_open()) {
        // Write header
        nor_file << "Sample";
    ...
    string command_str = "python3 cluster.py normalized_resi_matrix.csv data/" + label_file + " 42 " + to_string(freq_threshold) + " " + to_string(k) + " " + to_string(K) + " rebuttal_results_resi_local.csv";
