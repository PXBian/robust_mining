...
int main(int argv, char** argc) {
    ...
    // Choose top/buttom K as features
    vector<string> top_K_freq_resi_vec;
    int top_count = 0;
    // cout << "The freq_vec is " << endl;
    for (auto const &item : freq_vec) {
        // cout << "(" << item.first << "," << item.second << ") ";
        if(resi_set.find(item.first) != resi_set.end() && top_count < K && item.first.length() >= 5) {  //  
    ...
    cout << endl;
    cout << "The global Top-K frequent resilient patterns are: " << endl;
    for(const auto &item : top_K_freq_resi_vec) {
        cout << item << " ";
    }
    cout << endl;
    ...
    // // Choose random K as features
    // vector<pair<string, int>> random_freq_vec = freq_vec;
    // vector<string> random_K_freq_resi_vec;
    // random_device rd;
    // mt19937 g(rd());

    // shuffle(random_freq_vec.begin(), random_freq_vec.end(), g);
    // int random_count = 0;
    // for (auto const &item : random_freq_vec) {
    //     // cout << "(" << item.first << "," << item.second << ") ";
    //     if(resi_set.find(item.first) != resi_set.end()) {  //  
    //         random_K_freq_resi_vec.push_back(item.first);
    //         random_count++;
    //     }
    ...
    while (is_text.read(reinterpret_cast<char*>(&chr), 1)) {
		if(chr == '\n') {
            ...
            vector<INT> occ;
            for(auto const &pattern : top_K_freq_resi_vec) {
                unsigned char* target = const_cast<unsigned char*>(
    ...
    command_str = "python3 cluster.py normalized_resi_matrix.csv data/" + label_file + " 42 " + to_string(freq_threshold) + " " + to_string(k) + " " + to_string(K) + " rebuttal_results_resi.csv";
