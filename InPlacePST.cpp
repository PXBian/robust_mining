...
namespace PrioritySearchTree {
  ...
  InPlacePST::InPlacePST(PSTPoint* points, int n) {
	...
	// cout << "After initializing, the tree array is: " << endl;	// tree array is good at this point
	// for (int i = 0; i < n; i++) {
	// 	cout << "[" << tree[i].getX() << "," << tree[i].getY() << "] "; 
	// }
	// cout << endl;
    ...
    before = time(0);
    // heap_sort(tree,1,n);
	heap_sort(tree,0,n-1);
	// cout << "After heap sort, the tree array is " << endl;
	// for (int i = 0; i < n; i++) {
	// 	cout << "[" << tree[i].getX() << "," << tree[i].getY() << "] ";
	// }
	// cout << endl;
    ...
    for(int i = 0; i < h; i++) {
		buildLevel(i,n);
		// cout << "In level i=" << i << " the tree array is: " << endl;
		// for (int i = 0; i < n; i++) {
		// 	cout << "[" << tree[i].getX() << "," << tree[i].getY() << "] "; 
		// }
		// cout << endl;
	...
	cout << "After buildLevel, the tree array is: " << endl;
	for (int i = 0; i < n; i++) {
		cout << "[" << tree[i].getX() << "," << tree[i].getY() << "] "; 
    }
	cout << endl;
  ...
  void InPlacePST::buildLevel(int i, int n) {	// i is the current level, n is the number of nodes
    /////////////////////////////////////////////////////////////////////////////
	...
	// cout << "********************************" << endl;
	// cout << "the index of s is powerOf2(i+1)=" << powerOf2(i+1) << endl;
	// cout << "The para of loop " << i << " is h=" << h << ",A=" << A << ",k=" << k << ",s=[" << s.getX() << "," << s.getY() << "],k1=" << k1 << ",k2=" << k2 << ",k3=" << k3 << endl;
	// cout << "At the begining of the loop, the tree array is: " << endl;
	// for (int x = 0; x < n; x++) {
	// 	cout << "[" << getPoint(x).getX() << "," << getPoint(x).getY() << "] ";
	// }
	// cout << endl;
    /////////////////////////////////////////////////////////////////////////////
    ...
    // build the subtrees of size k1: SHOULD check the level is odd or even
    for(int j = 1; j <= k; j++) {
      	// Find point with maximum/minimum Y in range
      	indexOfMaxMinY = powerOf2(i)+(j-1)*k1;
		if (i % 2 ==0) {	// This level is even, choose the minimum Y
			for(int index = indexOfMaxMinY; index <= powerOf2(i)+j*k1-1; index++) {
				if(getPoint(index).yLessThan(getPoint(indexOfMaxMinY))) {
					indexOfMaxMinY = index;
				}
	  		}
		}
		else {		// This level is odd, choose the maximum Y
			for(int index = indexOfMaxMinY; index <= powerOf2(i)+j*k1-1; index++) {
				if(getPoint(index).yGreaterThan(getPoint(indexOfMaxMinY))) {
					indexOfMaxMinY = index;
				}
	  		}
		}
      	// cout << "j = " << j << ", indexOfMaxMinY = " << indexOfMaxMinY << ", powerOf2(i)+j-1 = " << powerOf2(i)+j-1 << endl;
		swap(indexOfMaxMinY,powerOf2(i)+j-1);
		// cout << "After this swap, the tree array is: " << endl;
		// for (int x = 0; x < n; x++) {
		// 	cout << "[" << getPoint(x).getX() << "," << getPoint(x).getY() << "] ";
		// }
		// cout << endl;
    ...
    if(k < powerOf2(i)) {
      	...
      	indexOfMaxMinY = powerOf2(i)+k*k1;
      	
		if (i % 2 ==0) {	// This level is even, choose the minimum Y
			for(int index = indexOfMaxMinY; index <= powerOf2(i)+k*k1+k2-1; index++) {
				if(getPoint(index).yLessThan(getPoint(indexOfMaxMinY)))
					indexOfMaxMinY = index;
			}
		}
      	...
      	// cout << "k = " << k << ", indexOfMaxMinY = " << indexOfMaxMinY << ", powerOf2(i)+k = " << powerOf2(i)+k << endl;
      	swap(indexOfMaxMinY,powerOf2(i)+k);
		// cout << "After this swap, the tree array is: " << endl;
		// for (int x = 0; x < n; x++) {
		// 	cout << "[" << getPoint(x).getX() << "," << getPoint(x).getY() << "] ";
		// }
		// cout << endl;
      	...
      	for(int j = 1; j <= powerOf2(i)-k-1; j++) {
			indexOfMaxMinY = m+(j-1)*k3;
			if (i % 2 ==0) {	// This level is even, choose the minimum Y
				for(int index = indexOfMaxMinY; index <= m+j*k3-1; index++) {
					if(getPoint(index).yLessThan(getPoint(indexOfMaxMinY))) {
						indexOfMaxMinY = index;
					}
				}
      		...
      		// cout << "m = " << m << ", j = " << j << ", indexOfMaxMinY = " << indexOfMaxMinY << ", powerOf2(i)+k+j = " << powerOf2(i)+k+j << endl;
			swap(indexOfMaxMinY,powerOf2(i)+k+j);
			// cout << "After this swap, the tree array is: " << endl;
			// for (int x = 0; x < n; x++) {
			// 	cout << "[" << getPoint(x).getX() << "," << getPoint(x).getY() << "] ";
			// }
	...
	// cout << "The para of inPlaceSort is powerOf2(i+1) = " << powerOf2(i+1) << ", n = " << n << endl;
    inPlaceSort(powerOf2(i+1),n,s);
	// cout << "After inPlaceSort, the tree array is: " << endl;
	// for (int x = 0; x < n; x++) {
	// 	cout << "[" << getPoint(x).getX() << "," << getPoint(x).getY() << "] ";
	// }
	// cout << endl;
...
//   PSTPoint InPlacePST::leftMostSE(coord_t xmin, coord_t ymax) {
//     PSTPoint best(POSITIVE_INFINITY,NEGATIVE_INFINITY);
//     int indexP = 1; // start at root
//     int indexQ = 1;
//     while(!isLeaf(indexP)) {
...
//   }
  ...
  void InPlacePST::explore(int indexP, coord_t ymin, vector<PSTPoint>* points) {
	...
	if(state == 0) {
	  ...
	  if(numberOfChildren(indexC) > 0 && getPoint(indexCl).getY() >= ymin) {
	    indexC = indexCl;
	  } else {
	    state = 1;
	  }
    ...
    void InPlacePST::exploreDown(int indexP, coord_t ymax, vector<PSTPoint>* points) {
		PSTPoint p = getPoint(indexP);
		// p is within query region
		if(p.getY() <= ymax) {
		int indexC = indexP;
		...
		// 0: intial value
		// 1: all elements of current & left child of current in T which intersect with query region Q have been reported
		// 2: all elements of current in T (intersect with query region Q) have been reported
		int state = 0;	
		while((indexC != indexP) || (state != 2)) {
			current = getPoint(indexC);
			if(state == 0) {
				...
				// if current has a left child and y(left child of current) >= ymin
				int indexCl = indexOfLeftChild(indexC);
				if(numberOfChildren(indexC) > 0 && getPoint(indexCl).getY() <= ymax) {
					indexC = indexCl;
				} else {
					state = 1;
				}
				...
				if(state == 1) {
					// if current has a right child and y(right child of current) >= ymin
					int indexCr = indexOfRightChild(indexC);
					if(numberOfChildren(indexC) == 2 && getPoint(indexCr).getY() <= ymax) {
						indexC = indexCr;
						state = 0;
						} else {
						state = 2;
  ...
  vector<PSTPoint>* InPlacePST::enumerate3SidedUp(coord_t xmin,
						coord_t xmax,
						coord_t ymin) {
    /////////////////////////////////////////////////////////////////////////////
    // Initialization                                                          //
  ...
  vector<PSTPoint>* InPlacePST::enumerate3SidedDown(coord_t xmin,
						coord_t xmax,
						coord_t ymax) {
    /////////////////////////////////////////////////////////////////////////////
    // Initialization                                                          //
	...
	// CASE 2: p has one child (must be left)
	  ...
	  if(xmin <= pl.getX() && pl.getX() <= xmax) {
	    ...
	    if(Lp && Rp) {
	      // Explore(p')
	      exploreDown(indexPp,ymax,points);
	    }
	    // If there is a point p' s.t.:
	    //   xmin <= x(p') <= xmax
	    else if(Lp) {
	...
	// CASE 3: p has two children
	  ...
	  // CASE 3B: left child is within query region
		...
		exploreDown(indexPp,ymax,points);
		// Explore(pr)
		exploreDown(indexPr,ymax,points);
	      }
	      // If there is a point p' s.t.:
	      //   xmin <= x(p') <= xmax
	      else if(Lp) {
	...
	// end EnumerateLeft(p)
		...
		// search from the left within the query region
			...
			// CASE 3B: left child is within query region
				...
				if(Rp) {
				// Explore(p'r)
				exploreDown(indexPpr,ymax,points);
				indexPp = indexPpl;
				}
				// Otherwise, there is no such point q'
				else {
	...
	// CASE 3: p has two children
	  ...
	  if(qr.getX() > xmax) {
	    ...
	    // CASE 3A(ii): left child is within query region
	      ...
	      if(Lp && Rp) {
		// Explore(q')
		exploreDown(indexQp,ymax,points);
	      }
	      // If there is a point q' s.t.:
	      //   xmin <= x(q') <= xmax
	      else if(Rp) {
	  ...
	  // CASE 3B: right child is within query region
		...
		exploreDown(indexQp,ymax,points);
		// Explore(ql)
		exploreDown(indexQl,ymax,points);
	      }
	      // If there is a point q' s.t.:
	      //   xmin <= x(q') <= xmax
	      else if(Rp) {
	...
	// CASE 3: q' has two children
	  ...
	  // CASE 3B: right child is within query region
	    ...
	    // CASE 3B(ii): left child must be within query region
	      ...
	      if(Lp) {
		// Explore(q'l)
		exploreDown(indexQpl,ymax,points);
		indexQp = indexQpl;
	      }
	      // Otherwise, there is no such point q'
	      else {
